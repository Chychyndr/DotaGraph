...
The SVG renderer keeps one persistent relationship graph in the DOM. Overview renders the complete published relationship edge set at very low contrast. Hover and Focus only change emphasis on those existing lines; selecting a hero never inserts a second edge set, substitutes endpoints, or creates a focus overlay. Up to five incoming and five outgoing relationships become prominent while unrelated heroes are dimmed in place. This preserves one continuous graph throughout the interaction without changing counter semantics.
